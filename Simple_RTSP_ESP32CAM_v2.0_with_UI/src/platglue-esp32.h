...
{
    int result = sockfd->beginPacket(destaddr, destport);
    if (!result) {
        printf("error beginning udp packet\n");
        return -1;
    }
    
    size_t written = sockfd->write((const uint8_t *)buf, len);
    if (written != len) {
        printf("error writing udp packet: wrote %d of %d bytes\n", written, len);
    }
    
    result = sockfd->endPacket();
    if (!result) {
